...
class MultinomialLogisticRegression(object):
    ...
    def initialize_weights(self, input_dim, num_classes, std_dev=1e-2):
        ...
        #############################################################################
        self.params['W'] = (np.multiply(np.random.randn(input_dim*num_classes), std_dev)).reshape(input_dim,num_classes)
    ...
    def train(self, X, y, learning_rate=1e-3, lambda_reg=1e-5, num_iters=100,
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            self.params['W'] = self.params['W']-np.multiply(learning_rate,grads['W'])
            self.params['b'] = self.params['b']-np.multiply(learning_rate,grads['b'])
            #########################################################################
            #                             END OF YOUR CODE                          #
            #########################################################################
